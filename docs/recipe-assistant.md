...
One structured request combines action routing and recipe research planning. It selects recipe editing, collections, online photos, generation from the recipe or generation from a food reference. Recipe plans either ask for a missing fact, search for a new foundation or reuse the editor's retained sources. A request to generate a new image using the user's photo takes the food-reference route. Ambiguous photo requests show explicit choices. The photo menu bypasses routing when the user has already selected an action. Photo actions operate separately from recipe evidence validation and only change the image plus its credit after review.
...
The editor also retains downloaded sources through recipe clarifications and failed generation. A question about a selected source keeps that exact recipe for the answer, including its servings. Follow-ups can skip web search and page downloads; routing and planning remain one request so users can change direction. Changed drafts, fresh research, new planning questions, non-recipe actions, Apply, Discard and Undo invalidate the retained context. Earlier user messages remain available within the bounded conversation; retry does not append a duplicate user message. Nothing is persisted outside the editor.

Equivalent quantity notation, such as `100`/`100.0` or `1/4`/`¼`, passes evidence checks when ingredient names and units match. Different amounts, units and unannotated method changes still fail. Planning trims surrounding whitespace before validation. Planning, search and adaptation review have output budgets that allow room for reasoning as well as the returned answer. Failures identify the stage and distinguish output limits, unfinished responses, unreadable answers and specific evidence checks. Long-press the error to copy its details; no raw provider text, keys or prompts are included.
...
Multi-turn transport tests replay the cream/stock sauce conversation through the production orchestration, including the initial yield question, source-specific clarification and acceptance of the published servings. They verify one search, a retained source and a reviewed proposal after seven total model calls across the three turns, versus eleven in the previous flow. Additional tests cover retry without research, stale-draft invalidation, equivalent quantities and output-limit diagnostics. These counts establish the removed network work; they do not predict live provider latency.
