...
One small structured request routes natural-language chat to recipe editing, collections, online photos, generation or photo enhancement. Ambiguous photo requests show explicit choices. The photo menu bypasses routing when the user has already selected an action. Photo actions operate separately from recipe evidence validation and only change the image plus its credit after review.
...
Chat starts as a compact input over the recipe editor on iOS 26, with Liquid Glass and a system material fallback. Focusing the input expands chat; tap or drag down its grabber to collapse. There is no Ask AI header or close control. Send and Stop use regular controls within the input's 44-point minimum height. The dock sits at the bottom safe area with narrower collapsed margins. The editor renders behind the glass, with extra scroll-content margin so its last fields remain reachable. Ingredients, individual ingredient and method-step forms, tags, collections and notes remain navigable while chat is open. Collapsing retains conversation, input, pending changes and undo history; ongoing requests continue. Stop or leaving the recipe editor cancels local work and ignores late results.

Discovery centres a glass input with warm window light, terracotta cookware, olive herbs and compact inspiration chips. Loading animates a stirring spoon, bubbling soup and rising steam, alongside the actual search stage and a Cancel action that preserves the prompt. Canvas motion pauses when inactive and becomes static with Reduce Motion. Results retain source labels, counts and decisions, with source information in the options menu. Red/green swipe feedback belongs only to the translated top card; the stationary stack stays neutral.
