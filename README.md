...
Discovery imports only published recipes, never generated ones. Editor chat can adapt a published base to your ingredients, preserving core quantities and explaining estimated seasonings and method changes for review; adaptations are not kitchen-tested. URL import prefers structured `Recipe` metadata and explicit ingredient headings. Vision recognizes recipe text in photos. Small formatting, tags and cookbook filter interpretation remain on-device. Manual capture and deterministic quantities remain available offline without a key; URL import needs internet but no key.
